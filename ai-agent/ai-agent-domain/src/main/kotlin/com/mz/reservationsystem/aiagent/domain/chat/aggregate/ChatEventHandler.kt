...
class ChatEventHandler : AggregateEventHandler<Chat, ChatEvent> {
    ...
    override fun apply(aggregate: Chat, event: ChatEvent): Chat {
    ...
    private fun newChat(aggregate: EmptyChat, event: ChatEvent): Chat {
        return when (event) {
            is ChatCreated -> aggregate.apply(event)
            else -> throw RuntimeException("Wrong event type $event for the empty chat aggregate")
        }
    }

    private fun existingChat(aggregate: UnknownCustomerChat, event: ChatEvent): Chat {
        return when (event) {
    ...
    private fun customerChat(aggregate: CustomerChat, event: ChatEvent): Chat {
        return when (event) {
